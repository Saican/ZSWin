...
{	
	...
	//
	// Wrapper methods for zhandler calls for locating windows
	//
	ZSWindow GetWindow(int index) { return zHandler ? ZSWindow(zHandler.GetWindow(index)) : null; }
	ZSWindow GetWindowByPriority(int priority) { return zHandler ? ZSWindow(zHandler.GetWindowByPriority(priority)) : null; }
	ZSWindow FindWindow(string WindowName) { return zHandler ? ZSWindow(zHandler.FindWindow(WindowName)) : null; }
	...
	*/
	...
	ui ZText FindText(string ControlName) 
	{ 
		for (int i = 0; i < GetTextSize(); i++) 
		{ 
			if (GetText(i).ControlName ~== ControlName) 
				return GetText(i); 
		} 
		return null; 
	}
	...
	ui ZShape FindShape(string ControlName) 
	{ 
		for (int i = 0; i < GetShapeSize(); i++) 
		{ 
			if (GetShape(i).ControlName ~== ControlName) 
				return GetShape(i); 
		} 
		return null; 
	}
	...
	ui ZButton FindButton(string ControlName)
	{
		for (int i = 0; i < GetButtonSize(); i++)
		{
			if (GetButton(i).ControlName ~== ControlName)
				return GetButton(i);
		}
		return null;
	}
